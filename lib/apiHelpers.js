...
/**
 * Wraps a Route Handler so any error it throws (a missing env var, a DB
 * connection failure, a malformed request body, etc.) is logged
 * server-side and turned into a JSON 500 response instead of an
 * uncaught exception.
 *
 * Without this, Next.js's production error handling sends back an
 * *empty* response body for an unhandled Route Handler error -- which is
 * why a client doing `await response.json()` on the result sees
 * "Unexpected end of JSON input" instead of a useful message. The real
 * error is still logged here (visible in `vercel logs` / the Vercel
 * dashboard); the client also gets `detail` so you can see it in the
 * browser Network tab while debugging. Drop `detail` from the response
 * before this app is public-facing, since it can leak internals.
 *
