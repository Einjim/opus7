...
// ("Content for Chemistry education will go here. console.log('Welcome to
// Chemistry education!')") -- clicking "شیمی" (Chemistry) from scripts.js's
...
// prefixed with "Chemistry"/"chemistry" so this file can never collide
// with math.js/biology.js/physics.js's copies of the same pattern -- see
...
const CHEMISTRY_CATEGORY = 'Chemistry';

function showChemistryPage() {
    fetch(`/quiz/get_quizzes_by_category/${encodeURIComponent(CHEMISTRY_CATEGORY)}`)
            ...
            const pageContent = `
                <div class="container" style="position: relative; height: 100vh; display: flex; align-items: center; justify-content: center;">
                    ...
                    <main style="display: flex; flex-direction: column; align-items: center;">
                        <h2 style="margin-bottom: 20px;">Chemistry Quizzes</h2>
                        <div class="chemistry-quiz-buttons" style="display: flex; flex-direction: column; align-items: center;">
                            ${quizzes.length > 0 ? generateChemistryQuizButtons(quizzes) : '<p>در حال حاضر آزمونی برای شیمی وجود ندارد.</p>'}
                        </div>
                    </main>
                </div>
            `;
...
function generateChemistryQuizButtons(quizzes) {
    return quizzes.map(quiz => `
        <button class="chemistry-quiz-button" data-quiz="${quiz}" style="margin: 5px;">${quiz}</button>
    `).join('');
}

function handleChemistryQuizButtonClick(event) {
    const button = event.target;
    if (!button.classList.contains('chemistry-quiz-button')) return;

    const quizName = button.getAttribute('data-quiz');
    localStorage.setItem('currentQuizName', quizName);
    localStorage.setItem('currentQuizCategory', CHEMISTRY_CATEGORY);
    ...
    bluePage.addEventListener('click', () => {
        fetch('/quiz/start_quiz', {
            ...
            body: JSON.stringify({ quiz: quizName, category: CHEMISTRY_CATEGORY }),
        })
        .then(response => response.json())
        .then(data => {
            if (data.questions && data.questions.length > 0) {
                ...
                document.getElementById('start-button').addEventListener('click', (event) => {
                    ...
                    if (chemistryQuizStarting) return;
                    chemistryQuizStarting = true;
                    event.stopPropagation();
                    ...
                    // variable, unreachable from showChemistryQuizCompleted()
                    // below. If the user finished the quiz early (before
                    // time ran out), this timer kept running in the
                    // background and could still fire showChemistryTimeOverPage()
                    ...
                    chemistryCheckTimeInterval = setInterval(() => {
                        const currentTime = new Date();
                        if (currentTime >= new Date(localStorage.getItem('quizEndTime'))) {
                            clearInterval(chemistryCheckTimeInterval);
                            showChemistryTimeOverPage();
                        }
                    }, 1000);

                    showChemistryQuestion(0);
                });
            } else {
                bluePage.innerHTML = `
                    <header style="position: absolute; top: 10px; left: 10px;">
                    ...
                    <h1>No Chemistry questions available for ${quizName}</h1>
                `;
                document.getElementById('back-btn-blue').addEventListener('click', (event) => {
                    event.stopPropagation();
                    showChemistryPage();
...
let chemistryQuizStarting = false;
let chemistryCheckTimeInterval = null;
let chemistrySubmitting = false;

function showChemistryQuestion(index) {
            ...
            <div class="options" style="margin-top: 20px; display: flex; flex-direction: column; align-items: flex-start;">
                ${options.map((option, i) => `
                    <label style="margin: 5px; padding: 10px; display: flex; align-items: center;">
                        <input type="checkbox" name="option" value="${i}" style="appearance: none; -webkit-appearance: none; width: 20px; height: 20px; border: 2px solid #333; border-radius: 50%; margin-right: 10px; position: relative;" onclick="toggleChemistryOption(this)">
...
function toggleChemistryOption(checkbox) {
...
function showChemistryQuizCompleted() {
    // BUG FIX: stop the 10-minute timer now -- see chemistryCheckTimeInterval's
    // comment above. Without this, finishing early could still trigger
    // showChemistryTimeOverPage() a few seconds/minutes later and silently
    // replace this results screen.
    if (chemistryCheckTimeInterval) {
        clearInterval(chemistryCheckTimeInterval);
        chemistryCheckTimeInterval = null;
    }
    ...
    // single quiz attempt.
    if (chemistrySubmitting) return;
    chemistrySubmitting = true;
    ...
    submitChemistryQuizResults(quizName, userAnswers, startTime, duration, totalQuestions)
        .then(data => {
...
function showChemistryTimeOverPage() {
...
function submitChemistryQuizResults(quizName, userAnswers, startTime, duration, totalQuestions) {
    const category = localStorage.getItem('currentQuizCategory') || CHEMISTRY_CATEGORY;
    // BUG FIX (see handleChemistryQuizButtonClick above): allQuestionIds is a
...
document.addEventListener('click', handleChemistryQuizButtonClick);